...
(function() {
	...
	function makeObjectArray(array, key, tagType) {
	    array = array || [];
	    if (array.length > 0 && !angular.isObject(array[0]) && tagType != 'array') {
	...
	function findInObjectArray(array, obj, key) {
	    ...
	    for (var i = 0; i < array.length; i++) {
	        ...
	        if ((angular.isObject(array[i]) && array[i][key].toLowerCase() === obj[key].toLowerCase()) ||
	        	(angular.isString(array[i]) && array[i].toLowerCase() === obj[key].toLowerCase())) {
	            item = array[i];
	            break;
	        }
	...
	tagsInput.directive('tagsInput', ["$timeout","$document","tagsInputConfig", function($timeout, $document, tagsInputConfig) {
	    function TagList(options, events) {
	        ...
	        getTagText = function(tag) {
	            return tag[options.displayProperty] || tag;
	        };

	        setTagText = function(tag, text) {
	        	if(angular.isString(tag)){ tag = text; }
	        ...
	        self.add = function(tag) {
	            ...
	            if(!angular.isString(tagText)){ return '';}
	            ...
	            if (tagIsValid(tag)) {
	                self.items.push(options.tagType == 'array' ? tag[options.displayProperty] : tag);
	    ...
	    return {
	        ...
	        link: function(scope, element, attrs, ngModelCtrl) {
	            ...
	            scope.getDisplayText = function(tag) {
	                return (tag[options.displayProperty] || tag).trim();
	            };
	            ...
	            scope.$watch('tags', function(value) {
	                scope.tags = makeObjectArray(value, options.displayProperty, options.tagType);
	                ...
	                .on('keydown', function(e) {
	                    ...
	                    if (shouldAdd) {
	                        ...
	                        if (tag && options.enableEditingLastTag) {
	                            scope.newTag.text = tag[options.displayProperty] || tag;
	                        }
	        ...
	        self.load = function(query, tags) {
	            ...
	            debouncedLoadId = $timeout(function() {
	                ...
	                promise.then(function(items) {
	                    ...
	                    items = makeObjectArray(items.data || items, options.tagsInput.displayProperty, options.tagType);
	...
	tagsInput.run(["$templateCache", function($templateCache) {
	    ...
	    "<div class=\"host\" tabindex=\"-1\" ti-transclude-append=\"\"><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item btn-info\" ng-repeat=\"tag in tagList.items track by track(tag)\" ng-class=\"{ selected: tag == tagList.selected }\"><span>{{getDisplayText(tag)}}</span> <a class=\"remove-button\" ng-click=\"tagList.remove($index)\">{{options.removeTagSymbol}}</a></li></ul><input class=\"input\" placeholder=\"{{options.placeholder}}\" tabindex=\"{{options.tabindex}}\" ng-model=\"newTag.text\" ng-change=\"newTagChange()\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ti-autosize=\"\"></div></div>"
	  );
